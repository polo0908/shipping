...
public class DetailListServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String sqlString="select id,clientName,address,volume,saildate,fromwhere,towhere,currency,yunfei,package,packagenum,totalGW,yunfeifs from products where id="+id;
...
//			System.out.println("出口货物明细单："+sqlString);
			...
			while (resultSet.next()) {
				...
				request.setAttribute("yunfeifs", resultSet.getString("yunfeifs"));
...
//				System.out.println(purnoall+"=======");
				...
				String sql2="select hscode,itemeng,itemchn,shopingmark,rate,quantity,trueprice,nw from items where proId="+id;
...
//					String shopingmark = res.getString("shopingmark");
					...
					if(flag==0){
					allContent.append(itemeng).append("-").append(itemchn).append(";").append(packagenum).append(packagess).append(";").append(quantity).append("PCS;").append(totalGW).append("KGS;").append(nw)
					.append("KGS;").append(df2.format(price/quant)).append(";").append("TOTAL:"+currency + df.format(price)+" <br/>");
					}else{
						allContent.append(itemeng).append("-").append(itemchn).append(";----;").append(quantity).append("PCS;----KGS;").append(nw)
						.append("KGS;").append(df2.format(price/quant)).append(";").append("TOTAL:"+currency + df.format(price)+" <br/>");
					}
					flag++;
				}
				request.setAttribute("allContent", allContent.toString());
				request.setAttribute("totalMoney",currency +" "+ df.format(totalPrice));
			}
		...
		RequestDispatcher homeDispatcher = request.getRequestDispatcher("view/detailList.jsp");
